...
class GridSpec:
    ...
    def indices(self, xy: np.ndarray, clip: bool = False) -> tuple[np.ndarray, np.ndarray]:
        points = np.asarray(xy, dtype=np.float64)
        columns = np.floor((points[:, 0] - self.minimum_x) / self.resolution).astype(np.int64)
        rows = np.floor((points[:, 1] - self.minimum_y) / self.resolution).astype(np.int64)
        if clip:
            rows = np.clip(rows, 0, self.height - 1)
            columns = np.clip(columns, 0, self.width - 1)
        elif np.any((rows < 0) | (rows >= self.height) | (columns < 0) | (columns >= self.width)):
            raise ValueError("One or more points lie outside the grid")
        return rows, columns

    def center_mesh(self) -> tuple[np.ndarray, np.ndarray]:
        x = self.minimum_x + (np.arange(self.width, dtype=np.float32) + 0.5) * self.resolution
        y = self.minimum_y + (np.arange(self.height, dtype=np.float32) + 0.5) * self.resolution
...
def infer_two_cell_rule(train_positions: np.ndarray) -> dict:
    ...
    for axis in (0, 1):
        ...
        for index in np.argsort(gaps)[-32:]:
            ...
            candidates.append((score, float(gaps[index]), axis, threshold, left_count, right_count))
...
def assign_cells(positions: np.ndarray, rule: dict) -> np.ndarray:
    ...
    return np.where(np.asarray(positions)[:, axis] <= threshold, lower, upper).astype(np.int64)
...
def make_grid_spec(positions: np.ndarray, resolution: float, margin: float) -> GridSpec:
    ...
    width, height = np.maximum(1, np.ceil((maximum - minimum) / resolution).astype(np.int64))
    return GridSpec(
...
def build_geometry_maps(
    ...
    valid = (np.abs(relative_angle) <= sector_half_angle_degrees) & (distance <= maximum_distance)
    identity = np.zeros((cell_count, spec.height, spec.width), dtype=np.float32)
    identity[cell_id] = 1.0
    return {
        "distance": np.clip(distance / maximum_distance, 0.0, 1.5).astype(np.float32)[None],
...
def build_bev_features(points_xyz: np.ndarray, spec: GridSpec) -> np.ndarray:
    ...
    columns = np.floor((points[:, 0] - spec.minimum_x) / spec.resolution).astype(np.int64)
    rows = np.floor((points[:, 1] - spec.minimum_y) / spec.resolution).astype(np.int64)
    inside = (rows >= 0) & (rows < spec.height) & (columns >= 0) & (columns < spec.width)
...
def _kmeans(points: np.ndarray, cluster_count: int, seed: int, iterations: int = 100) -> np.ndarray:
    values = np.asarray(points, dtype=np.float64)
    rng = np.random.default_rng(seed)
    centers = np.empty((cluster_count, values.shape[1]), dtype=np.float64)
    centers[0] = values[rng.integers(len(values))]
    nearest_square = ((values - centers[0]) ** 2).sum(axis=1)
    for index in range(1, cluster_count):
        probabilities = nearest_square / max(nearest_square.sum(), 1e-30)
        centers[index] = values[rng.choice(len(values), p=probabilities)]
        distance = ((values - centers[index]) ** 2).sum(axis=1)
        nearest_square = np.minimum(nearest_square, distance)
    ...
    for _ in range(iterations):
        ...
        for index in range(cluster_count):
            ...
            if len(members):
                ...
                centers[index] = values[rng.integers(len(values))]
...
def spatial_fold_ids(
    ...
    folds = np.full(len(positions), -1, dtype=np.int64)
    for cell_id in np.unique(cell_ids):
        indices = np.flatnonzero(cell_ids == cell_id)
        folds[indices] = _kmeans(positions[indices, :2], fold_count, seed + int(cell_id) * 1009)
    return folds


def grid_collision_statistics(rows: np.ndarray, columns: np.ndarray, cell_ids: np.ndarray) -> list[dict]:
    output = []
    for cell_id in np.unique(cell_ids):
...
def test_support_summary(
    ...
    for cell_id in np.unique(train_cells):
        train_xy = np.asarray(train_positions[train_cells == cell_id, :2], dtype=np.float64)
        test_xy = np.asarray(test_positions[test_cells == cell_id, :2], dtype=np.float64)
