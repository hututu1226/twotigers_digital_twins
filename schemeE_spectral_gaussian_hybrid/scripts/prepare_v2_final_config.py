...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare full-data Scheme E-v2 config")
    parser.add_argument("--selection", default="reports/generated/v2_attempt_selection.json")
    parser.add_argument("--output", default="configs/v2_final_selected.json")
    ...
    policy = json.loads(
        Path(f"reports/generated/v2_attempt{attempt}_policy.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    config["hybrid_final"]["output_dir"] = "artifacts/v2/final/hybrid"
    config["hybrid_final"]["train_decoder"] = bool(
        config["hybrid"].get("train_decoder", False)
    )
    config["hybrid_final"]["decoder_learning_rate_scale"] = float(
    ...
    config["inference"].update(
        {
            "checkpoint": "artifacts/v2/final/hybrid/best.pt",
            "output_path": "outputs/v2/Round2_Test_Channel.npy",
            "report_path": "reports/generated/v2_final_inference.json",
