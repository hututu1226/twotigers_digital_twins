...
from .splits import fold_summary, spatial_block_folds
...
def preprocess_dataset(config: dict, force: bool = False) -> dict:
    ...
    try:
        ...
        if not bool(section.get("allow_small_dataset", False)):
            raise
        fold_count = int(section["fold_count"])
        validation_masks = np.zeros((fold_count, len(train_positions)), dtype=bool)
        for cell_id in np.unique(train_cells):
            indices = np.flatnonzero(train_cells == cell_id)
            order = indices[np.argsort(train_positions[indices, 0])]
            for rank, index in enumerate(order):
                validation_masks[rank % fold_count, index] = True
    log_power, outage = _channel_power(
        data_root / "Round2_Train_Channel.npy",
        len(train_positions),
        int(section.get("channel_chunk_size", 8)),
    )
    mesh_vertices, mesh_faces = read_ascii_ply_mesh(data_root / "Round2_Map.ply")
    point_cloud = mesh_vertices.astype(np.float32)

    gaussian_field = build_rf_gaussians(
        mesh_vertices,
        mesh_faces,
        int(section.get("rf_gaussian_count", 10_070)),
    )
    gaussian_path = artifact_dir / "rf_gaussians.npz"
    gaussian_field.save(gaussian_path)
    stations = np.asarray(setup["X"], dtype=np.float32)
    train_geometry_features = extract_geometry_features(
        train_positions,
        train_cells,
        stations,
        gaussian_field,
        corridor_samples=int(section.get("corridor_samples", 24)),
        fresnel_radius_meters=float(section.get("fresnel_radius_meters", 2.0)),
    )
    test_geometry_features = extract_geometry_features(
        test_positions,
        test_cells,
        stations,
        gaussian_field,
    ...
    spectral_folds = spatial_block_folds(
        train_positions,
        train_cells,
        fold_count=int(section.get("spectral_fold_count", 8)),
        tile_meters=float(section.get("spectral_tile_meters", 24.0)),
    ...
    np.savez_compressed(
        ...
        spectral_folds=spectral_folds,
        log_power=log_power,
        outage=outage,
        train_geometry_features=train_geometry_features,
        test_geometry_features=test_geometry_features,
    )
    manifest = {
        "version": 2,
        "setup": setup,
        ...
        "point_cloud_faces": int(len(mesh_faces)),
        "rf_gaussians": int(len(gaussian_field.centers)),
        "rf_gaussian_path": gaussian_path.name,
        "geometry_feature_count": int(train_geometry_features.shape[1]),
        "geometry_feature_names": feature_names(),
        "spectral_folds": fold_summary(spectral_folds, train_cells),
