...
def spectral_feature_shapes(shape: ChannelShape, proxy_count: int = 24) -> dict[str, tuple[int, ...]]:
...
def _normalize_distribution(value: torch.Tensor, dimensions: tuple[int, ...]) -> torch.Tensor:
...
def channel_spectral_targets(
    ...
    proxy = angle_power.reshape(-1, proxy_count, group_size, shape.m_v, shape.m_h).mean(dim=2)
    ...
    delay_power = delay.abs().square().float().reshape(
    ...
    ue_log_energy = torch.where(outage[:, None], torch.zeros_like(ue_log_energy), ue_log_energy)
...
def decode_pas_log(
    ...
    if pas_log.shape[-1] != expected:
        raise ValueError(f"Expected PAS feature width {expected}, got {pas_log.shape[-1]}")
...
def decode_pdp_log(pdp_log: torch.Tensor, shape: ChannelShape) -> torch.Tensor:
    ...
    if pdp_log.shape[-1] != expected:
        raise ValueError(f"Expected PDP feature width {expected}, got {pdp_log.shape[-1]}")
