    ...
    torch.save(
        {
            ...
            "training_stage": "transport_residual_context_v3",
        },
        path,
    )
...
def train_context_model(config: dict, resume: bool = False) -> dict:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        anneal_epochs = max(int(section.get("router_temperature_anneal_epochs", epochs)), 1)
            ...
            with autocast_context(device, amp):
                ...
                if torch.any(nonzero):
                    ...
                    terms["base_spectrum_latent"] = functional.smooth_l1_loss(
                        outputs["spectrum_base"][nonzero].float(), target_spectrum[nonzero]
                    )
                ...
                minimum_neighbors = float(section.get("router_minimum_effective_neighbors", 8.0))
                terms["router_diversity"] = (
                    functional.relu(
                        minimum_neighbors - outputs["router_effective_neighbors"].float()
                    )
