...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify Scheme D formal artifacts")
    ...
    try:
        ...
        with np.load(encoded) as source:
            total = int(np.prod(source["spectrum_shape"]) + np.prod(source["phase_shape"]))
            available = int(source["available"].sum()) if "available" in source.files else 0
        if total != 30_720:
        ...
        if args.stage == "fold0":
            ...
            if array.shape != (500, 256, 4, 192) or array.dtype != np.complex64:
                raise ValueError(f"invalid final output shape={array.shape}, dtype={array.dtype}")
            if not np.isfinite(array).all():
                raise ValueError("final output contains NaN or Inf")
            details["test_output"] = {"shape": list(array.shape), "bytes": output.stat().st_size}
