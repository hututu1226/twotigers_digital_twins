...
仓库已经有 Scheme G 的 `.gitattributes`：
...
```bash
git lfs install
git add -f schemeG_multiscale_spectral_context/artifacts/fold0/context/best.pt
git commit -m "Add Scheme G Fold0 checkpoint via LFS"
git push origin 0821_schemeG
```
...
- 系统盘只放临时依赖；
- 数据、已有权重和最终备份放 `/root/autodl-fs` 持久盘；仓库可继续使用当前 `/root/autodl-tmp` 工作区；
...
```bash
cd /root/autodl-tmp
git clone --branch 0821_schemeG --single-branch \
...
```bash
git config --global http.version HTTP/1.1
git clone --depth 1 --branch 0821_schemeG \
  https://github.com/hututu1226/twotigers_digital_twins.git \
  twotigers_digital_twins
```
...
ln -s /root/autodl-fs/schemeF_0820_20260820/schemeC_full_resolution_context/artifacts/fold0/autoencoder/best.pt \
...
```bash
cd /root/autodl-tmp/twotigers_digital_twins
ln -s /root/autodl-fs/schemeF_0820_20260820/Round2_Map Round2_Map
```
...
python -m pip install -r schemeG_multiscale_spectral_context/requirements.txt
...
```text
context_architecture = multiscale_spectral_context_v1
total_latent_elements = 30720
full_latent_linear_layers = []
full_resolution_check = true
context_parameters = 20614659（小版本调整时可略有变化）
router.spectrum_top_k = 32
router.spectrum_candidate_top_k = 96
router.detail_top_k = 8
router.detail_candidate_top_k = 32
router.power_top_k = 16
router.token_top_k = 4
```
...
```bash
cat /root/autodl-fs/schemeG_0821/unattended_status.txt
find /root/autodl-fs/schemeG_0821 -maxdepth 2 -type f -printf '%p %s bytes\n' | sort
```
...
schemeG_results_*.tar.gz
schemeG_results_*.tar.gz.sha256
reports/
logs/
```

实例自动关机后 Jupyter 连接断开是预期行为，不等于结果丢失；结果已经放在 `/root/autodl-fs/schemeG_0821`。
...
```bash
cd /root/autodl-fs/schemeG_0821
...
```powershell
scp -P 端口 root@主机:/root/autodl-fs/schemeG_0821/Round2_Test_Channel.npy `
  "D:\华为算法大赛复赛\autodl_results\20260821_schemeG\"

scp -P 端口 root@主机:/root/autodl-fs/schemeG_0821/schemeG_results_*.tar.gz* `
  "D:\华为算法大赛复赛\autodl_results\20260821_schemeG\"
```
...
```powershell
cd "D:\华为算法大赛复赛\autodl_results\20260821_schemeG"
Get-FileHash .\schemeG_results_*.tar.gz -Algorithm SHA256
```
...
```text
reports/schemeG_fold0_EXPERIMENT_REPORT.md
reports/schemeG_final_EXPERIMENT_REPORT.md
...
- `spectrum_router_distance_meters`：大尺度路由是否仍被带到 20 m 以外；
- `detail_router_distance_meters`：细节搬运是否超过测试场景通常的 6--12 m 支撑范围；
- `detail_token_effective_neighbors`：Top4 是否过早塌成 Top1；
- `best_cellwise_combined`：BS0/BS1 独立阈值和频谱投影后的严格组合分数；
- `cellwise_gain_vs_global`：独立策略相对统一策略的离线增益；
- `recommended_for_submission`：低于 `0.65` 时为 false；
- final 报告没有 test accuracy，因为比赛测试标签不可见。

## 18. 计费边界

脚本最大允许一次主 Fold、一次修复 Fold和一次 final。配置墙钟保护约为 `3.5 + 3.0 + 3.5` 小时；早停后常见总耗时约 `4--8` 小时，最坏保护值约 10 小时。
