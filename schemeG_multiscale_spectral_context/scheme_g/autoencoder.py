...
class BranchEncoder(nn.Module):
    def __init__(
        self, input_channels: int, stem_channels: int, latent_channels: int
    ) -> None:
        super().__init__()
...
class StructuredAngleDelayAutoencoder(nn.Module):
    ...
    def decode(
        ...
        return normalize_angle_delay(
            self.decoder(torch.cat([spectrum, phase], dim=1)).float()
        )
...
class HighFidelityDecoder(nn.Module):
    ...
    def forward(self, spectrum: torch.Tensor, detail: torch.Tensor) -> torch.Tensor:
...
class MetricHighFidelityAutoencoder(nn.Module):
    ...
    def decode(
        ...
        spectrum = spectrum_latent.reshape(-1, *self.spectrum_shape.tensor_shape)
        if phase_latent is None:
            detail = spectrum.new_zeros((len(spectrum), *self.phase_shape.tensor_shape))
        else:
    ...
    def __init__(
        ...
        if residual:
            layers.extend(
                [
                    nn.GroupNorm(
                        _groups(output_channels), output_channels, affine=False
                    ),
                    nn.GELU(),
                    *[
...
class FactorizedResidualDecoder(nn.Module):
    ...
    def _envelope(self, log_power: torch.Tensor) -> torch.Tensor:
        ...
        return (
            ...
            .expand(-1, -1, 2, -1, -1, -1)
            .reshape(
                amplitude.shape[0], 2 * self.complex_channels, *amplitude.shape[2:]
            )
        )
...
class FactorizedResidualAutoencoder(nn.Module):
    ...
    def decode(
        ...
        return self.decode_components(spectrum_latent, phase_latent, detail_scale)[
            "prediction"
        ]
