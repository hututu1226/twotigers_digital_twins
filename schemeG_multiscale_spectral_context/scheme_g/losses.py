...
def energy_weighted_angle_delay_mse(
    ...
    weights = (1.0 + float(emphasis) * normalized.sqrt()).clamp_max(
        float(maximum_weight)
    )
    weights = weights[:, :, None].expand(-1, -1, 2, -1, -1, -1)
    weights = weights.reshape_as(target)
    return (
        (prediction.float() - target.float()).square() * weights
...
def complex_coherence_loss(
    ...
    coherence = functional.cosine_similarity(predicted, expected, dim=1, eps=1e-8)
...
def energy_weighted_complex_direction_loss(
    ...
    cosine = (predicted * expected).sum(dim=2) / (predicted_norm * expected_norm)
