...
from .context_model import FullResolutionContextField, spectrum_chart
...
def build_context_model(
    ...
    shape = ChannelShape.from_setup(repository.manifest["setup"])
...
def _model_inputs(
    ...
    prior = repository.spectral_prior_values(query["indices"], test=test)
    if prior is not None:
        result.update(
            {
                "query_prior_pas": _tensor(prior["pas_log"], device),
                "query_prior_pdp": _tensor(prior["pdp_log"], device),
                "query_prior_uncertainty": _tensor(prior["uncertainty"], device),
            }
        )
    return result
...
def predict_indices(
    ...
    diagnostics = {
        ...
        for name in (
            ...
            "router_distance",
            "detail_router_entropy",
            "detail_router_top1_mass",
            "detail_router_effective_neighbors",
            "detail_router_distance",
            "power_router_effective_neighbors",
            "power_router_distance",
            "spectrum_warp",
            "detail_warp",
            "warp_saturation",
            "router_temperature",
...
def evaluate_context_model(
    ...
    device_cache: dict[str, object] | None = None,
    soft_outage_strength: float | list[float] | None = None,
...
def evaluate_context_thresholds(
    ...
    configured_soft_strength = (
        repository.config["context"].get("soft_outage_strength", 0.0)
        if soft_outage_strength is None
        else soft_outage_strength
    )
    soft_strengths = (
        [float(value) for value in configured_soft_strength]
        if isinstance(configured_soft_strength, (list, tuple, np.ndarray))
        else [float(configured_soft_strength)]
    )
    if not soft_strengths:
        raise ValueError("At least one soft outage strength is required")
    prior_alpha = float(
    ...
    policies = [
        (soft_strength, threshold)
        for soft_strength in soft_strengths
        for threshold in thresholds
    ]
    accumulators = [ChannelMetricAccumulator(shape) for _ in policies]
    channels = np.load(
    ...
    for start in range(0, len(target_indices), decode_batch_size):
        ...
        for (soft_strength, threshold), metrics in zip(
            policies, accumulators, strict=True
        ):
    ...
    for (soft_strength, threshold), metrics in zip(policies, accumulators, strict=True):
        ...
        result.update(
            {
                ...
                "metric_nonzero_count": int(metrics.nonzero_count),
                "nmse_numerator": float(metrics.nmse_numerator),
                "nmse_denominator": float(metrics.nmse_denominator),
                ...
                "router_distance_meters": float(
                    outputs["router_distance"].mean() * repository.maximum_distance
                ),
                "detail_router_effective_neighbors": float(
                    outputs["detail_router_effective_neighbors"].mean()
                ),
                "detail_router_top1_mass": float(
                    outputs["detail_router_top1_mass"].mean()
                ),
                "detail_router_distance_meters": float(
                    outputs["detail_router_distance"].mean()
                    * repository.maximum_distance
                ),
                "power_router_effective_neighbors": float(
                    outputs["power_router_effective_neighbors"].mean()
                ),
                "power_router_distance_meters": float(
                    outputs["power_router_distance"].mean()
                    * repository.maximum_distance
                ),
                ...
                "power_uncertainty_width": float(
                    outputs["power_uncertainty_width"].mean()
                ),
    ...
    torch.save(
        {
            ...
            "training_stage": "multiscale_spectral_context_v1",
        },
        path,
    )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            with autocast_context(device, amp):
                ...
                minimum_detail_neighbors = float(
                    section.get("detail_router_minimum_effective_neighbors", 3.0)
                )
                spectrum_diversity = (
                    functional.relu(
                        minimum_neighbors
                ...
                detail_diversity = (
                    functional.relu(
                        minimum_detail_neighbors
                        - outputs["detail_router_effective_neighbors"].float()
                    )
                    / max(minimum_detail_neighbors, 1.0)
                ).square()
                terms["router_diversity"] = 0.5 * (
                    spectrum_diversity.mean() + detail_diversity.mean()
                )
...
def load_context_checkpoint(
    ...
    if "router_temperature" in checkpoint:
        model.set_router_temperature(float(checkpoint["router_temperature"]))
