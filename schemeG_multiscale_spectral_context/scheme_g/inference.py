...
def _cell_parameter(value: object, cell_id: int, name: str) -> float:
    if isinstance(value, dict):
        key = str(int(cell_id))
        if key not in value:
            raise ValueError(f"{name} has no value for cell {cell_id}")
        return float(value[key])
    if isinstance(value, (list, tuple, np.ndarray)):
        if not 0 <= int(cell_id) < len(value):
            raise ValueError(f"{name} has no value for cell {cell_id}")
        return float(value[int(cell_id)])
    return float(value)
...
def generate_test_channels(
    ...
    outage_threshold: float | list[float] | None = None,
) -> dict:
    ...
    soft_policy = config["inference"].get("soft_outage_strength", 0.0)
    prior_policy = config["inference"].get("spectral_prior_alpha", 0.0)
    thresholds = np.asarray(
        [
            _cell_parameter(threshold_policy, int(cell), "outage_threshold")
            for cell in cells
        ],
        dtype=np.float32,
    )
    if np.any((thresholds <= 0.0) | (thresholds >= 1.0)):
        raise ValueError("Every outage_threshold must lie in the open interval (0, 1)")
    predicted_outage = outputs["outage_probability"] >= thresholds
    ...
    for start in range(0, len(selected), decode_batch_size):
        ...
        cell_tensor = torch.from_numpy(cells[start:stop]).to(
        ...
        local_phase_mean = (
            phase_mean[cell_tensor] if phase_mean.ndim == 2 else phase_mean
        )
        ...
        phase = (
            torch.from_numpy(outputs["phase"][start:stop]).to(device) * local_phase_std
            + local_phase_mean
        )
        ...
        prior = repository.spectral_prior_values(selected[start:stop], test=True)
        batch_cells = cells[start:stop]
        if any(
            _cell_parameter(prior_policy, int(cell), "spectral_prior_alpha") > 0.0
            for cell in np.unique(batch_cells)
        ):
            if prior is None:
                raise ValueError(
                    "spectral_prior_alpha is positive but test priors are missing"
                )
            for cell_id in np.unique(batch_cells):
                local = np.flatnonzero(batch_cells == cell_id)
                alpha = _cell_parameter(
                    prior_policy, int(cell_id), "spectral_prior_alpha"
                )
                if alpha <= 0.0:
                    continue
                local_tensor = torch.from_numpy(local).to(
                    device=device, dtype=torch.long
                )
                projected = soft_shape_projection(
                    channel.index_select(0, local_tensor),
                    torch.from_numpy(prior["pas_log"][local]).to(device),
                    torch.from_numpy(prior["pdp_log"][local]).to(device),
                    shape,
                    alpha,
                    proxy_count=int(
                        config["context"]
                    ...
                    iterations=int(
                        config["context"].get("spectral_prior_iterations", 1)
                    ),
                    minimum_scale=float(
                        config["context"].get("spectral_prior_minimum_scale", 0.75)
                    ),
                    maximum_scale=float(
                        config["context"].get("spectral_prior_maximum_scale", 1.35)
                    ),
                )
                channel.index_copy_(0, local_tensor, projected)
        for cell_id in np.unique(batch_cells):
            local = np.flatnonzero(batch_cells == cell_id)
            local_tensor = torch.from_numpy(local).to(device=device, dtype=torch.long)
            masked = apply_outage_policy(
                channel.index_select(0, local_tensor),
                outputs["outage_probability"][start:stop][local],
                _cell_parameter(threshold_policy, int(cell_id), "outage_threshold"),
                _cell_parameter(soft_policy, int(cell_id), "soft_outage_strength"),
            )
            channel.index_copy_(0, local_tensor, masked)
    ...
    summary = {
        ...
        "outage_threshold": threshold_policy,
        "soft_outage_strength": soft_policy,
        "spectral_prior_alpha": prior_policy,
