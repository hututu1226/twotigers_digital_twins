...
from scheme_g.config import load_config, save_json
...
def percentiles(values: list[float]) -> dict[str, float]:
    result = np.percentile(
        np.asarray(values, dtype=np.float64), [0, 25, 50, 75, 95, 100]
    )
    return {
        name: float(value)
        for name, value in zip(
            ("minimum", "p25", "median", "p75", "p95", "maximum"), result
        )
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Measure Scheme G training-mask support"
    )
