...
BACKUP_ROOT="${BACKUP_ROOT:-/root/autodl-fs/schemeG_latest}"
...
backup() {
  mkdir -p "$BACKUP_ROOT"
  shopt -s nullglob
  archives=(schemeG_results_*.tar.gz schemeG_results_*.tar.gz.sha256)
...
status RUNNING "Scheme G formal pipeline is running"
set -o pipefail
