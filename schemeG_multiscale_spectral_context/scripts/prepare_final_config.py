...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Select Scheme G full-data epochs and outage threshold"
    )
    ...
    parser.add_argument("--epoch-multiplier", type=float, default=1.35)
    ...
    config["context"]["spectral_prior"]["oof_path"] = (
        "../schemeE_spectral_gaussian_hybrid/artifacts/fold0/"
        "spectral_teacher/oof_priors.npz"
    )
    ...
    threshold = outage_report.get(
        "best_threshold_by_cell", outage_report["best_threshold"]
    )
    soft_strength = outage_report.get(
        "best_soft_strength_by_cell",
        outage_report.get("best_soft_strength", 0.0),
    )
    prior_alpha = outage_report.get(
        "best_spectral_prior_alpha_by_cell",
        outage_report.get("best_spectral_prior_alpha", 0.0),
    )
