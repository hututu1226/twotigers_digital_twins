...
CONTEXT_DIR="$(python -c 'import sys; from scheme_g.config import load_config; print(load_config(sys.argv[1])["context"]["output_dir"])' "$CONFIG")"
CHECKPOINT="$CONTEXT_DIR/best.pt"
mkdir -p "$CONTEXT_DIR" logs reports/generated

if [[ ! -s artifacts/preprocessed_scheme_g/manifest.json ]] || ! \
  python -c 'import numpy as np; s=np.load("artifacts/preprocessed_scheme_g/metadata.npz"); assert "train_geometry_features" in s.files and s["train_geometry_features"].shape[1] == 71'; then
...
if [[ -s "$CHECKPOINT" && -s "$CONTEXT_DIR/summary.json" ]]; then
  echo "Existing Scheme G attempt found at $CONTEXT_DIR; reusing it."
...
python scripts/apply_cellwise_policy.py \
  --evaluation "$CONTEXT_DIR/evaluation.json" \
  --scan "$CONTEXT_DIR/outage_scan.json"
