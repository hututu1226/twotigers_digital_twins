...
quality-gated V4 基线严格复现为 PAS=`0.570384`、PDP=`0.759130`、NMSE=`1.004499`、Score=`0.631581`。固定幅度组合得到 PAS=`0.574170`、PDP=`0.760325`、NMSE=`1.003398`、Score=`0.633628`，净增 `+0.002047`。零修正复现误差仅为 `1.30e-9`。V4 与该候选的 target-informed 二专家 oracle 为 `0.640268`。耗时 `41.11` 秒。

### Interpretation

保留 V4 相位、只叠加局部幅度修正的组合方向确实比此前携带 Teacher seed 相位的候选合理，PAS、PDP 和 NMSE 三项同时改善；但它只追回了固定最低晋级增益的约三分之二，二专家理想上限也远低于目标。这个结果是一个可复现的小候选，不足以证明继续扫描强度或训练 gate 有收益。

### Decision

`DROP_PRIMARY / KEEP_MINOR_ARTIFACT`。保持 `0.631581` 为预注册协议下的主基线；保留 `0.633628` 预测文件用于分布切片诊断，但不扫描 `K`、strength 或 Router。

## L0-023

### Hypothesis

Scheme1 旧验证仅 `0.3925`、官方却有 `0.62`，而 Scheme E 严格 Fold0 为 `0.6316`、旧线上只有 `0.59`。两次线上/线下反转可能来自验证洞与 500 个测试点的实际几何难度或无线环境不一致。

### Minimal Experiment

不训练模型，也不使用测试信道。只使用训练/测试位置、基站标签和已预处理的 71 维 RF 几何，比较旧 Scheme1 验证、strict Fold0、五个现有 Fold 与 test 的同基站支撑距离、密度和域分类 AUC。固定按 cell 与最近邻距离对 Fold0 指标重加权，再复查 L0-022。

### Result

最近非零训练点距离中位数分别为：旧 Scheme1 验证 `21.594 m`、strict Fold0 `6.578 m`、test `6.199 m`。strict-to-test 中位数差只有 `0.380 m`，support-only 域分类最大 AUC=`0.595935`；旧 Scheme1 与 test 的距离差达到 `15.395 m`。

然而，去掉绝对坐标后，包含基站距离/方位、局部表面、走廊、Fresnel 和遮挡的 link/environment 域分类最大 AUC 仍为 `0.996294`。现有五个 Fold 的该 AUC 都在 `0.993` 以上，Fold1 虽在邻点难度上最接近 test，也没有解决环境偏移。

L0-022 在按 test 的 cell/邻点距离重加权后，基线/候选为 `0.629556/0.631871`，增益 `+0.002316`。strict BS0/BS1 增益为 `+0.000184/+0.004139`，仍未达到固定的总体 `+0.003` 门槛。

### Interpretation

旧 Scheme1 的离线验证洞远大于真实测试洞，这能解释其离线分数过低，但不能证明初赛方案本身更强。当前 Fold0 已正确模拟“周围训练点有多远”，却没有覆盖测试点所在的无线环境；因此继续只按单一 Fold0 调参存在选错方向的风险。
