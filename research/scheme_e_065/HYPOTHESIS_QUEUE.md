...
|---:|---|---|---|---|---|---:|---|---|
| 1 | 极端高误差样本可由新的可靠回退候选改善。 | 最差 5% 占 67.27% 误差能量；L0-022 二专家 oracle 仅 `0.640268`。 | `+0.003` 到 `+0.010`。 | 新候选需把 oracle 推到 `>0.66`。 | 新候选产生后先算联合 oracle。 | <=0.2 h | oracle `<0.66`。 | 只有过线后才允许严格 OOF gate。 |
...
- nearest test-neighborhood holdout：L0-025 support AUC=`0.647896`、最近距离差 `0.207 m`，但 link/environment AUC 仍为 `0.960860`，未达到固定 `<=0.896294` 门槛。
