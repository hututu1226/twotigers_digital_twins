...
def generate_test_channels(
    ...
    soft_strength = float(config["inference"].get("soft_outage_strength", 0.0))
    prior_alpha = float(config["inference"].get("spectral_prior_alpha", 0.0))
    ...
    for start in range(0, len(selected), decode_batch_size):
        ...
        if prior_alpha > 0.0:
            ...
            channel = soft_shape_projection(
                ...
                prior_alpha,
                proxy_count=int(
                    config["context"].get("spectral_prior", {}).get("proxy_count", 24)
                ),
                iterations=int(config["context"].get("spectral_prior_iterations", 1)),
                minimum_scale=float(
        ...
        channel = apply_outage_policy(
            channel,
            outputs["outage_probability"][start:stop],
            threshold,
            soft_strength,
        )
