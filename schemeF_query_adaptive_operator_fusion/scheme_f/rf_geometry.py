...
def read_ascii_ply_mesh(path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    with source.open("r", encoding="ascii") as handle:
        ...
        try:
            ...
            raise ValueError(
                f"PLY vertex properties must contain x/y/z: {source}"
            ) from error
        ...
        for _ in range(face_count):
            ...
            anchor = indices[0]
            triangles.extend(
                (anchor, indices[i], indices[i + 1]) for i in range(1, count - 1)
            )
...
def _morton_order(points: np.ndarray) -> np.ndarray:
    ...
    quantized = np.clip(np.rint((points - minimum) / span * 1023.0), 0, 1023).astype(
        np.uint64
    )
    keys = (
...
def _neighbors(
    tree: object | None, centers: np.ndarray, point: np.ndarray, radius: float
) -> np.ndarray:
    if tree is not None:
...
def _local_features(
    field: GaussianField, tree: object | None, point: np.ndarray
) -> list[float]:
    output: list[float] = []
    for radius in LOCAL_RADII_METERS:
        ...
        output.extend(
            [
                ...
                float(
                    np.sqrt(np.sum(weights * (dz - np.sum(weights * dz)) ** 2)) / radius
                ),
...
def _corridor_profiles(
    ...
    for index, fraction in enumerate(samples):
        ...
        facing[index] = float(
            np.sum(weight * normal_alignment) / max(weight.sum(), 1e-12)
        )
