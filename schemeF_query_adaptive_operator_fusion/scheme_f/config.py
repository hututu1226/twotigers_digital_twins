...
def load_config(path: str | Path) -> dict:
    ...
    reuse_geometry = config.get("preprocessing", {}).get("reuse_geometry_metadata")
    if reuse_geometry:
        value = Path(reuse_geometry)
        if not value.is_absolute():
            config["preprocessing"]["reuse_geometry_metadata"] = str(
                (project_root / value).resolve()
            )
    prior = config.get("context", {}).get("spectral_prior", {})
    for key in ("oof_path", "test_path"):
        if not prior.get(key):
            continue
        value = Path(prior[key])
        if not value.is_absolute():
            prior[key] = str((project_root / value).resolve())
