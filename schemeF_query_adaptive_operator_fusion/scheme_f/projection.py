...
def _pas_projection(
    ...
    current = power.reshape(-1, proxy_count, group_size, shape.m_v, shape.m_h).mean(
        dim=2
    )
...
def soft_shape_projection(
    ...
    """Apply only mild PAS/PDP corrections and preserve each sample's total power."""
    if not 0.0 <= alpha <= 1.0:
        raise ValueError("spectral prior alpha must lie in [0, 1]")
    if alpha == 0.0 or iterations == 0:
        return channel
    target_proxy, _ = decode_pas_log(pas_log, shape, proxy_count)
    target_pdp = decode_pdp_log(pdp_log, shape)
    original_power = channel.abs().square().float().mean(dim=(1, 2, 3))
    projected = channel
    for _ in range(int(iterations)):
        ...
        projected = _pdp_projection(projected, target_pdp, minimum_scale, maximum_scale)

    def restore_power(value: torch.Tensor) -> torch.Tensor:
        current = value.abs().square().float().mean(dim=(1, 2, 3))
        scale = torch.sqrt(original_power.clamp_min(1e-30) / current.clamp_min(1e-30))
        return value * scale[:, None, None, None].to(value.dtype)

    projected = restore_power(projected)
    blended = (1.0 - float(alpha)) * channel + float(alpha) * projected
    return restore_power(blended)
