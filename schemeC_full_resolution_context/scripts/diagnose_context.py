...
from scheme_c.config import choose_device, load_config, seed_everything
from scheme_c.context_data import ContextRepository
from scheme_c.context_diagnostics import run_diagnostics
from scheme_c.context_training import load_context_checkpoint
from scheme_c.data import balanced_limit, load_metadata, split_indices
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Diagnose which Scheme C Context component limits validation score"
    )
    parser.add_argument("--config", default="configs/fold0_5090.json")
    parser.add_argument("--checkpoint")
    parser.add_argument("--encoded")
    parser.add_argument(
        "--output-dir", default="artifacts/fold0/context_diagnostics"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    for label, path in (("Context checkpoint", checkpoint_path), ("encoded latent", encoded_path)):
        if not path.is_file():
