...
def percentile_summary(values: np.ndarray) -> dict[str, float]:
    ...
    if not array.size:
        return {name: 0.0 for name in ("minimum", "p25", "median", "p75", "p95", "maximum", "mean")}
...
def routing_summary(outputs: dict[str, np.ndarray]) -> dict[str, dict[str, float]]:
    ...
    return {
        name: percentile_summary(outputs[name])
        for name in names
        if name in outputs
    }
    ...
    report.update(
        {
            ...
            "power_mae_z": float(np.mean(np.abs(power_error))) if len(power_error) else 0.0,
            "power_rmse_z": float(np.sqrt(np.mean(power_error**2))) if len(power_error) else 0.0,
            "power_mae_log10": (
...
def evaluate_channel_copy(
    ...
    for start in range(0, len(targets), size):
        ...
        target = torch.from_numpy(
            np.ascontiguousarray(target_channels[start:stop])
        ).to(device)
...
def write_markdown_summary(path: str | Path, report: dict) -> None:
    ...
    lines = [
        "# Scheme C Context Diagnostic Report",
        "",
...
def run_diagnostics(
    ...
    print("Loading the validation channels once to avoid repeated disk reads...", flush=True)
    ...
    print("[2/4] Replacing Spectrum, Detail, Power, and outage with truth...", flush=True)
    oracle_definitions = {
    ...
    print("[3/4] Measuring nearest-latent and low-dimensional power baselines...", flush=True)
    metadata = repository.metadata
    ...
    if power_neighbor_count < 1:
        raise ValueError("Every validation cell needs at least one non-outage observation")
    ...
    if include_counterfactuals:
        print("[4/4] Running no-Warp and weaker Router-prior counterfactuals...", flush=True)
        definitions = {
            "no_warp": (True, 1.0),
